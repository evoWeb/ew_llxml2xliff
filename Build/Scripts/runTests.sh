...
case ${TEST_SUITE} in
    ...
    checkGitSubmodule)
        ...
        ;;
    checkGruntClean)
        COMMAND="find 'typo3/sysext' -name '*.js' -not -path '*/theme_camino/*' -not -path '*/Fixtures/*' -not -path '*/Documentation/*' -exec rm '{}' + && cd Build; npm ci || exit 1; node_modules/grunt/bin/grunt build; cd ..; git add *; git status; git status | grep -q \"nothing to commit, working tree clean\""
    ...
    lintHtml)
        ...
        ;;
    lintPhp)
        COMMAND="php -v | grep '^PHP'; find . -name \\*.php -not -path './vendor/*' -not -path './bin/*' -not -path './public/*' -not -path './var/*' -not -path './typo3temp/*' -not -path './.cache/*' -not -path './Build/node_modules/*' -print0 | xargs -0 -n1 -P"'$(nproc 2>/dev/null || echo 4)'" php -dxdebug.mode=off -l >/dev/null"
